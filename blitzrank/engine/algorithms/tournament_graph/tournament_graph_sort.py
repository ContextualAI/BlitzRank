...
class TournamentGraphSort:
    ...
    async def sort_async(self) -> TournamentGraphSortResult:
        finalized_nodes = []
        num_rounds = 0
        nodes_for_next_match = self.items[: self.k]
        while len(finalized_nodes) < self.num_top_nodes_to_output:
            ...
            nodes_for_match = nodes_for_next_match
            (
                round_output,
                edges_added,
                oracle_result,
            ) = await self.organize_match_and_update_graph_async(nodes_for_match)
            ...
            await self._emit_round_log_async(
                self._build_round_log(
                    ...
                    nodes_for_match=nodes_for_match,
            ...
            nodes_for_next_match = tournament_progress.nodes_for_match
            finalized_nodes = tournament_progress.finalized_nodes
    ...
    def check_for_infinite_loop(
        ...
        """
        Raise error if the same match is scheduled consecutively, indicating a bug.
        ...
        """
        same_schedule = set(tournament_progress.nodes_for_match) == set(
            nodes_for_next_match
        )
        if same_schedule:
            raise RuntimeError(
